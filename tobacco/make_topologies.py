...
def get_topols_db(url):
    ...
    try:
        ...
        print("Could not connect to the database web:")
        print(url)
        exit()
...
def topols_config(cgd_filename="nets.cgd", consider_2D=False):
    """
    Configure the topology database.

    Parameters:
    consider_2D : bool
        consider 2D topologies (disabled by default)
    """
    # SETTINGS BY USER
    start_time = time.time()
    tol = 1E-2  # tolerance for distances
    scale = 10  # scale lattice constants by this factor
    cgd_filename = os.path.join(db, cgd_filename)

    # outputs templates
    tmpl3D = os.path.join(db, "template_database")
    tmpl2D = os.path.join(db, "template_2D_database")
    tmpl_errors = os.path.join(db, "template_errors")

    # Make folders to store topology CIFs
    if not os.path.exists(tmpl3D):
        os.mkdir(tmpl3D)
    if not os.path.exists(tmpl_errors):
        os.mkdir(tmpl_errors)
    if not os.path.exists(tmpl2D) and consider_2D:
        os.mkdir(tmpl2D)

    # Internal settings
    vnames = [  # names of vertices
        'V', 'Er', 'Ti', 'Ce', 'S',
        'H', 'He', 'Li', 'Be', 'B',
        'C', 'N', 'O', 'F', 'Ne',
        'Na', 'Mg', 'Al', 'Si', 'P',
    ...
    dummyb_list = [
        'cqe', 'cqv', 'dhb', 'krv', 'krvd', 'krw', 'krwd',
        'sdc', 'sdm', 'sdp', 'sdq', 'sdw', 'sdy', 'tdr',
        'tdw', 'tdx', 'tdy'
    ]
    ...
    for i in range(0, len(topologies_all)):
        ...
        try:
            pm_structure = Structure.from_spacegroup(
                group, lattice_vectors, sym_collection, basis_collection
            )
        except:
            warnings.warn(
        ...
        n_supercells = [
           np.ceil(extend/cellpars[0]),
           np.ceil(extend/cellpars[1]),
           np.ceil(extend/cellpars[2])
        ]
            ...
            for bond_dist in unique_bond_dists:
                ...
                for edge2_index in edge_overlap_indices:
                    ...
                    for possible_image in possible_images:
                        ...
                        dummy_atom = Structure(pm_structure.lattice.matrix, [edge_center_name],[dummy_pos],coords_are_cartesian=True)
        ...
        if threedim:
            with open(os.path.join(tmpl3D, topology+'.cif'), 'w') as w:
                w.write(top_text+cellpar_text+pos_text+bond_text)
        else:
            with open(os.path.join(tmpl2D, topology+'.cif'), 'w') as w:
                w.write(top_text+cellpar_text+pos_text+bond_text)
