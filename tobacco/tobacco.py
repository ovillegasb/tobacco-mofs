...
from tobacco.write_cifs import write_check_cif, write_cif, bond_connected_components, distance_search_bond, fix_bond_sym, merge_catenated_cifs
from tobacco.scale_animation import scaling_callback_animation, write_scaling_callback_animation, animate_objective_minimization
...
def run_template(template):
	...
	for net in ct2g(template):
		...
		nodes_path = os.path.join(database, "nodes")
		node_cns = [(cncalc(node, 'nodes'), node) for node in os.listdir(nodes_path)]
...
if __name__ == '__main__':
	...
	for d in ['template_database', 'nodes', 'edges']:
		try:
			os.remove(os.path.join(d,'.DS_Store'))
		except:
			pass
	
	templates = sorted(os.listdir('template_database'))
